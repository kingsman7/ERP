...
export class ErpBackupService {
  ...
  private checkAndExecuteScheduledBackup() {
    ...
    if (now >= nextRun && !this.isBackingUp()) {
      ...
      this.createBackup('SCHEDULED', 'Respaldo automático programado del sistema Helameb')
        .then(result => {
          if (result.success) {
  ...
  async createBackup(
    ...
    description = 'Respaldo completo de la base de datos Helameb',
    ...
    try {
      ...
      const fullPayload: ErpFullBackupPayload = {
        ...
        system: 'Helameb Enterprise Suite',
        checksum: '',
        metadata: backupMeta,
        data: {
          ...snapshotData,
      ...
      if (options?.downloadJson || this.scheduleConfig().autoDownloadJson) {
        this.downloadBackupAsJson(fullPayload, `Helameb_Respaldo_${backupCode}_${dateStr.slice(0, 8)}.json`);
      }
    ...
    try {
      ...
      if (!payloadString) {
        ...
        if (local && local.payloadJson) {
          this.triggerBrowserDownload(local.payloadJson, `Helameb_Respaldo_${local.backupCode}.json`);
          return true;
        }
      ...
      const fileName = `Helameb_Respaldo_${parsed.metadata?.backupCode || backupId}.json`;
      ...
      reader.onload = async (event) => {
        try {
          ...
          if (!parsedPayload.data || typeof parsedPayload.data !== 'object') {
            resolve({ success: false, recordsRestored: 0, error: 'El archivo no contiene un formato de respaldo válido de Helameb' });
            return;
          }
